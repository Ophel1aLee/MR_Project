...
# Print the mean and standard deviation of the given data
...
# Make a histogram of the given data. If if the number of bins or binsize isn't given, set the number of bins to sqrt(n)
...
# Analyze the number of vertices and faces from the cache file of the given database
...
# Create a histogram of distances from the barycenter of each shape to the world origin
...
# Perform PCA to get the eigenvectors, and create histograms of the dotr products e1.x, e2.y, and (e1xe2).z
...
# Get the largest dimension of the AABBs of the shapes and make a histogram of those lengths
def analyze_scale():
...
# Perform the fliiping test on every shape and tally up the positive and negative values for x, y, and z
...
def main(props: list[str]):
    all_props = ["sample_count", "edge_variance", "translation", "rotation", "scale", "flip"]
    ...
    operations = zip(all_props, all_functions)

    print("======================")

    # Only perform the operations whose name appears in the comma separated list
    for (p, f) in operations:
        if p in props:
            f()
            print("======================")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version', help='Version of the database to be analyzed (original, resampled, normalized) (default=original)', default='original')
    parser.add_argument('--properties', help='Comma-separated list of properties to be analyzed (sample_count, edge_variance, translation, rotation, scale, flip)(default=all)',
    ...
    props = "".join(args.properties.split()).split(',')
